...
"[project]/Desktop/calaya-taskly/src/views/dashboards/HOD/HODEditTask.jsx [app-client] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
function HODEditTask() {
    ...
    const fetchTask = (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$calaya$2d$taskly$2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useCallback"])({
        "HODEditTask.useCallback[fetchTask]": async ()=>{
            if (!taskId) return;
            try {
                ...
                if (res.ok) {
                    ...
                    const hodIds = data.assignments?.filter({
                        "HODEditTask.useCallback[fetchTask]": (a)=>a.user?.role === "HOD"
                    }["HODEditTask.useCallback[fetchTask]"]).map({
                        "HODEditTask.useCallback[fetchTask]": (a)=>a.userId
                    }["HODEditTask.useCallback[fetchTask]"]) ?? [];
                    const staffIds = data.assignments?.filter({
                        "HODEditTask.useCallback[fetchTask]": (a)=>a.user?.role === "Staff"
                    }["HODEditTask.useCallback[fetchTask]"]).map({
                        "HODEditTask.useCallback[fetchTask]": (a)=>a.userId
                    }["HODEditTask.useCallback[fetchTask]"]) ?? [];
                    const deptSet = new Set(data.assignments?.map({
                        "HODEditTask.useCallback[fetchTask]": (a)=>a.user?.department
                    }["HODEditTask.useCallback[fetchTask]"]).filter(Boolean) ?? []);
                    setFormData({
                        title: data.title ?? "",
            ...
            async function loadOptions() {
                try {
                    ...
                    if (deptRes.ok) setDepartments((await deptRes.json()).map({
                        "HODEditTask.useEffect.loadOptions": (d)=>d.name
                    }["HODEditTask.useEffect.loadOptions"]));
...
_s(HODEditTask, "xV2c4YbAwliHXslTSVrdLJu3Os8=", false, function() {
    return [
...
__turbopack_context__.k.register(_c2, "HODEditTask");
if (typeof globalThis.$RefreshHelpers$ === 'object' && globalThis.$RefreshHelpers !== null) {
    __turbopack_context__.k.registerExports(__turbopack_context__.m, globalThis.$RefreshHelpers$);
}
}),
...
//# sourceMappingURL=Desktop_calaya-taskly_src_85a0a5a3._.js.map