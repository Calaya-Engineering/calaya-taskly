...
export default function HODEscalations() {
  ...
  const handleReassign = (id) => {
    alert(`Reassigning escalation ${id}`);
  };

  const handleExtend = (id) => {
    const newDate = prompt('Enter new deadline (YYYY-MM-DD):');
    if (newDate) {
      alert(`Deadline extended to ${newDate} for escalation ${id}`);
    }
  };

  const handleResolve = (id) => {
    alert(`Marking escalation ${id} as resolved`);
  };

  const handleEscalate = (id) => {
    alert(`Escalating ${id} to MD`);
  };
        ...
        {view === "cards" ? (
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            {filteredEscalations.map((escalation) => (
              <Card key={escalation.id} className={`p-6 hover:shadow-md hover:-translate-y-0.5 transition-all ${escalation.actionRequired ? "border-l-4" : ""}`}
                ...
                <div className="flex items-start justify-between gap-4 mb-4">
                  ...
                  >
                    <span className="text-xl">{getTypeIcon(escalation.type)}</span>
                  </div>
                </div>
                ...
                <div className="mt-6 flex items-center gap-2">
                  <Link to={`/hod-dashboard/task/${escalation.taskId}`} className="flex-1">
                    <button
                      className="w-full px-4 py-2.5 rounded-2xl font-semibold text-sm text-white shadow-sm active:scale-[0.99] transition"
                      style={{ backgroundColor: "var(--secondary-blue)" }}
                    >
                      View Task
                    </button>
                    ...
                    <tr key={escalation.id} className={`hover:bg-gray-50/70 transition ${escalation.actionRequired ? "bg-red-50/20" : ""}`}>
                      ...
                      <td className="px-5 py-3 text-right whitespace-nowrap">
                        <div className="flex items-center justify-end gap-2">
                          <Link to={`/hod-dashboard/task/${escalation.taskId}`}>
                            <button
