...
export default function MDEvents() {
  ...
  const handleEditClick = (e, eventId) => {
    e.preventDefault();
    e.stopPropagation();
    navigate(`/md-dashboard/edit-event/${eventId}`);
  };
          ...
          >
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-5">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <Link to="/md-dashboard/create-event">
                  <button
                    ...
                    <tbody className="divide-y divide-gray-200/70 text-[12.5px]">
                      {listEvents.map((event) => (
                        <tr
                          ...
                          onClick={() => navigate(`/md-dashboard/event/${event.id}`)}
                        >
                            ...
                            {event.meetingLink ? (
                              ...
                              <button
                                onClick={(e) => {
                                  e.preventDefault();
                                  e.stopPropagation();
                                  navigate(`/md-dashboard/event/${event.id}`);
                                }}
                                className="px-3 py-1.5 rounded-xl text-[12px] font-semibold text-white shadow-sm active:scale-[0.99] transition"
                                style={{ backgroundColor: "var(--secondary-blue)" }}
                              >
