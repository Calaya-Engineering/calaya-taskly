    ...
    const fetchCounts = useCallback(async () => {
        ...
        try {
            const results = await Promise.allSettled([
                ...
                fetchWithAuth("/api/notifications?unread=true&compact=true&limit=100").then((r) =>
                    r.ok ? r.json() : []
                ),
                // Escalated tasks
                fetchWithAuth("/api/tasks?compact=true&limit=100").then((r) =>
                    r.ok ? r.json() : []
                ),
                // Announcements
                fetchWithAuth("/api/announcements?compact=true&limit=100").then((r) =>
                    r.ok ? r.json() : []
                ),
                // Tenders
                fetchWithAuth("/api/tenders?compact=true&limit=100").then((r) =>
                    r.ok ? r.json() : []
                ),
            ]);
            ...
            const escalatedTasks = Array.isArray(allTasks)
                ? allTasks.filter((t) => t.escalated && t.escalatedAt)
                : [];

            // Pending approval tasks
            const pendingApprovalCount = Array.isArray(allTasks)
            ...
            if (pathname !== escalationsPath) {
                newBadges[escalationsPath] = countNewSince(
                    escalatedTasks,
                    escalationsPath,
                    "escalatedAt"
                );
