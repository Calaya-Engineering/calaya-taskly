...
export default function StaffNotifications() {
  ...
  const totalPages = useMemo(
    () => Math.max(1, Math.ceil(filteredNotifications.length / NOTIFICATIONS_PAGE_SIZE)),
    [filteredNotifications.length]
  );

  const pagedNotifications = useMemo(() => {
    const start = (currentPage - 1) * NOTIFICATIONS_PAGE_SIZE;
    return filteredNotifications.slice(start, start + NOTIFICATIONS_PAGE_SIZE);
  }, [filteredNotifications, currentPage]);

  const virtualWindow = useMemo(() => {
    const count = pagedNotifications.length;
    const visibleCount = Math.ceil(NOTIFICATIONS_TABLE_VIEWPORT_HEIGHT / NOTIFICATIONS_ROW_HEIGHT);
    const startIndex = Math.max(0, Math.floor(scrollTop / NOTIFICATIONS_ROW_HEIGHT) - NOTIFICATIONS_OVERSCAN);
    const endIndex = Math.min(count, startIndex + visibleCount + NOTIFICATIONS_OVERSCAN * 2);
    return {
      rows: pagedNotifications.slice(startIndex, endIndex),
      topSpacer: startIndex * NOTIFICATIONS_ROW_HEIGHT,
      bottomSpacer: Math.max(0, (count - endIndex) * NOTIFICATIONS_ROW_HEIGHT),
    };
  }, [pagedNotifications, scrollTop]);
  ...
  }, [filter, selectedPriority, selectedType, searchTerm]);
              ...
              >
                {filteredNotifications.length === 0 ? (
                  ...
                  <>
                    ...
                    {virtualWindow.rows.map((notification) => {
            ...
            <Card className="p-6">
              ...
              <div className="mt-4 space-y-3">
                {["URGENT", "HIGH", "MEDIUM", "IMPORTANT", "NORMAL"].map((priority) => {
                  ...
                  const percentage = totalNotificationsCount
                    ? ((count / totalNotificationsCount) * 100).toFixed(1)
                    : "0.0";
