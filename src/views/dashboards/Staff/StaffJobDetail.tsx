...
const getStatusTone = (status?: string | null) => {
  switch (status) {
    ...
    case "PENDING":
    case "PENDING_HOD_APPROVAL": return "warn";
    case "PENDING_MD_APPROVAL": return "info";
      ...
      >
        <div
          style={{
            ...
            animation: "staff-job-spin 0.75s linear infinite",
          }}
        />
        <style>{`@keyframes staff-job-spin { to { transform: rotate(360deg); } }`}</style>
        <p style={{ fontWeight: 700, fontSize: "16px", color: "var(--primary-blue)", margin: 0, textAlign: "center" }}>
          Changing job to <span style={{ color: "var(--secondary-blue)" }}>{labelFormatted}</span>
        </p>
...
export default function StaffJobDetail() {
  ...
  const handleStatusChange = async (newStatus: string) => {
    ...
    try {
      ...
      setTaskData(data);
      toast.success(`Job status updated to ${getTaskStatusLabel(data.status || newStatus)}`);
    } catch {
        ...
        <div className="space-y-6">
          <Card className="p-12 text-center">
            ...
            <p className="text-gray-600 mt-2">
              No job data is available for this job right now.
            </p>
            <button
  ...
  return (
    <Layout menuItems={StaffMenuItems} userRole="Staff">
      ...
      <div className="space-y-6">
        ...
        {activeTab === "updates" && (
          <Card className="p-6">
            <SectionTitle title="Updates" subtitle="Live job data from the system" />
