...
export default function HODEditTask() {
  ...
  useEffect(() => {
    async function loadOptions() {
      try {
        ...
        const allDepartments = deptRes.ok ? (await deptRes.json()).map((d) => d.name) : [];
        const me = meRes.ok ? await meRes.json() : null;
        setDepartments(me?.managedDepartments?.length ? me.managedDepartments : allDepartments);
    ...
    return (
      <Layout menuItems={HODMenuItems} userRole="HOD">
        <Card className="p-12 text-center">
          ...
          <Link href="/hod-dashboard/tasks" className="mt-4 inline-block px-6 py-3 rounded-2xl font-semibold text-white" style={{ backgroundColor: "var(--primary-blue)" }}>
            Back to Tasks
          </Link>
        </Card>
      </Layout>
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        <Card className="overflow-hidden">
          <div className="p-6 md:p-8" style={{ background: "linear-gradient(135deg, rgba(44,75,155,0.10) 0%, rgba(109,198,223,0.18) 50%, rgba(237,50,55,0.06) 100%)" }}>
            <Link href={`/hod-dashboard/task/${taskId}`} className="text-sm text-gray-600 hover:text-gray-800 mb-4 inline-block">
              ← Back to Task
            </Link>
        ...
        <form onSubmit={handleSubmit}>
          <Card className="p-6 space-y-6">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mb-3">Forward to whole departments, specific HODs, or individual employees in your department(s).</p>
