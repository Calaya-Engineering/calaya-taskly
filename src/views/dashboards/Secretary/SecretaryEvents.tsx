...
export default function SecretaryEvents() {
  ...
  const filteredEvents = useMemo(() => {
    return events
      .filter((event) => {
        ...
        // Use ISO string comparison — today >= today, future > today
        const isUpcoming = eventISO >= todayISO; // includes today
        const isPast = eventISO < todayISO;      // strictly before today
  ...
  const weekEvents = useMemo(() => {
    ...
    return events
      .filter((e) => {
        const d = safeDate(e.startAt);
        return d ? d >= now && d <= weekFromNow : false;
      })
      .sort((a, b) => new Date(a.startAt).getTime() - new Date(b.startAt).getTime());
  }, [events, now]);
            ...
            {filteredEvents.map((event) => {
              ...
              return (
                <Card
                  ...
                  onClick={() =>
                    isAnn
                      ? router.push(`/secretary-dashboard/announcement/${event.dbId}`)
                      : router.push(`/secretary-dashboard/event/${event.dbId}`)
                  }
                >
                  <div className="p-6">
                    ...
                    <div className="flex items-center justify-between pt-3 border-t border-gray-200/70 mb-4">
                      ...
                      {/* Today badge */}
                      {toISODate(safeDate(event.startAt) || now) === todayISO && (
