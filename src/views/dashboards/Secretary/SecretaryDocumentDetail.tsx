...
export default function SecretaryDocumentDetail() {
  ...
  const handleAddComment = () => {
    ...
    toast.success(`Comment submitted${isInternalComment ? ' (Internal)' : ''}`);
    setNewComment('');
    ...
    return (
      <Layout menuItems={SecretaryMenuItems} userRole="Secretary">
        <Card className="p-12 text-center">
          ...
          <button
            onClick={() => router.push("/secretary-dashboard/documents")}
                      ...
                      <div>
                        ...
                        <div className="space-y-2">
                          {document.linkedTasks.map((taskId, index) => (
                            <Link key={index} href={`/secretary-dashboard/task/${taskId}`}>
                  ...
                  <div className="mt-6 space-y-4">
                    {versions.map((version, index) => (
                      ...
                      >
                        ...
                        {version.version !== document.version && (
                          <button
                            onClick={() => toast.info(`Downloading version ${version.version}`)}
