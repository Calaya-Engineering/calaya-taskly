...
const getJobDepartmentDisplay = (task: TaskData): string => {
      ...
      >
        <div
          style={{
            ...
            animation: "md-job-spin 0.75s linear infinite",
          }}
        />
        <style>{`@keyframes md-job-spin { to { transform: rotate(360deg); } }`}</style>
...
export default function MDJobDetail() {
  ...
  const fetchJob = useCallback(async () => {
    if (!jobId) return;
    try {
      const res = await fetchWithAuth(`/api/tasks/${jobId}`);
      if (res.ok) {
        ...
        <div className="space-y-6">
          <Card className="p-12 text-center">
            ...
            <p className="text-gray-600 mt-2">
              No job data available. Create jobs from the jobs list or select an existing job.
            </p>
            <button
              onClick={() => router.push("/md-dashboard/jobs")}
      ...
      <div className="space-y-6">
            ...
            <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-5">
              <div>
                ...
                <div className="flex flex-wrap items-center gap-2 mb-3">
                  ...
                  <Pill tone="info">JOB</Pill>
                  <Pill>{getJobDepartmentDisplay(taskData)}</Pill>
                </div>
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <Link href={`/md-dashboard/edit-task/${jobId}`}>
                  <button
        ...
        {activeTab === "overview" && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            {/* Job Info & Actions - Right Column */}
            <div className="lg:col-span-1 space-y-6">
              <Card className="p-6">
                <SectionTitle title="Job Information" />

                <div className="mt-6 space-y-4">
                  <InfoRow label="Department" value={getJobDepartmentDisplay(taskData)} />
        ...
        {activeTab === "updates" && (
          <Card className="p-6">
            ...
            <div className="mt-6 p-8 text-center text-gray-500">
              <p>Add comments and updates to jobs (coming soon).</p>
            </div>
          </Card>
        )}
        ...
        {/* Quick Actions */}
        <Card className="p-6">
          <SectionTitle title="Quick Actions" subtitle="Common job operations" />

          <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { icon: "💼", label: "Create Sub-job", color: "var(--primary-blue)" },
              { icon: "🔄", label: "Reassign Job", color: "var(--secondary-blue)" },
